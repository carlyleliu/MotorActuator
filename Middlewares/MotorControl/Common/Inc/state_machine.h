...
typedef enum {
    ...
    FAULT_NOW = 10,             /*!< Persistent state, the state machine can be moved from
                                 ...
                                 stm_fault_processing method. This method also manage
 ...
 */
typedef struct {
    State_t state;           /*!< Variable containing state machine current
                              state */
    uint16_t fault_now;      /*!< Bit fields variable containing faults
                              currently present */
    uint16_t fault_occurred; /*!< Bit fields variable containing faults
...
void stm_init(state_machine_t* phandle);

/* It submits the request for moving the state machine into the state state */
bool stm_next_state(state_machine_t* phandle, State_t state);

/* It clocks both hw and SW faults processing and update the state machine accordingly */
State_t stm_fault_processing(state_machine_t* phandle, uint16_t set_errors, uint16_t reset_errors);

/* Returns the current state machine state */
State_t stm_get_state(state_machine_t* phandle);

/**
 * It reports to the state machine that the fault state has been
 *        acknowledged by the user.
 */
bool stm_fault_acknowledged(state_machine_t* phandle);
 ...
 */
uint32_t stm_get_fault_state(state_machine_t* phandle);
