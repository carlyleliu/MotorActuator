...
#define BB_REG_BIT_SET(regAddr, bit_number) \
    *(uint32_t*)(0x42000000 + (((uint32_t)regAddr - 0x40000000) << 5) + (bit_number << 2)) = (uint32_t)(0x1u)
#define BB_REG_BIT_CLR(regAddr, bit_number) \
    (*(uint32_t*)(0x42000000 + (((uint32_t)regAddr - 0x40000000) << 5) + (bit_number << 2)) = (uint32_t)(0x0u))
#define BB_REG_BIT_READ(regAddr, bit_number) \
    (*(uint32_t*)(0x42000000 + (((uint32_t)regAddr - 0x40000000) << 5) + (bit_number << 2)))
/** @} */
...
#define M3     (uint8_t)(0x2)  /*!< motor 1.*/
#define M4     (uint8_t)(0x3)  /*!< motor 2.*/
...
/**
 * @brief  ad_conv_t type definition, it is used by pwmc_adc_set_sampling_time method of PWMnCurrFdbk class for user defined A/D regular conversions
 */
typedef struct {
    uint8_t Channel;    /*!< Integer channel number, from 0 to 15 */
    uint8_t sampl_time; /*!< Sampling time selection, ADC_SampleTime_nCycles5
                          in case of STM32F10x, n= 1, 3, ...; */
} ad_conv_t;

/**
 * @brief  sensor_type_t type definition, it's used in BusVoltageSensor and TemperatureSensor component parameters
 * structures to specify whether the sensor is real or emulated by SW
 */
typedef enum { REAL_SENSOR, VIRTUAL_SENSOR } sensor_type_t;

/**
 * @brief  doutput_state_t type definition, it's used by dout_set_output_state method of DigitalOutput class to specify
 * the required output state
 */
typedef enum { INACTIVE, ACTIVE } doutput_state_t;

/**
 * @brief  stc_modality_t type definition, it's used by stc_set_control_mode and stc_get_control_mode methods in
...
typedef struct {
    ...
    int32_t const_1q;
    int32_t const_2;
} IMff_tuning_struct_t, ff_tuning_struct_t;

/**
 * @brief  Current references source type, internal or external to FOCDriveClass
 */
typedef enum { INTERNAL, EXTERNAL } curr_ref_source_t;
...
typedef struct {
    ...
    qd_t iqd_hf;                   /**< @brief Stator current on stator reference frame alfa-beta*/
    qd_t iqd;                      /**< @brief Stator current on rotor reference frame qd */
    qd_t iqd_ref;                  /**< @brief Stator current on rotor reference frame qd */
    int16_t user_id_ref;           /**< @brief User value for the Idref stator current */
    ...
    int16_t teref;                 /**< @brief Reference torque */
    int16_t el_angle;              /**< @brief Electrical angle used for reference frame transformation  */
    uint16_t code_error;           /**< @brief error message */
    curr_ref_source_t drive_input; /**< @brief It specifies whether the current reference source must be
                                    *         #INTERNAL or #EXTERNAL*/
} foc_vars_t, *ptr_foc_vars_t;
...
} low_side_outputs_function_t,
*ptr_low_side_outputs_function_t;
