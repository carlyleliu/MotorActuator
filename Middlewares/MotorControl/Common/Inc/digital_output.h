 ...
 */
typedef struct {
    doutput_state_t output_state; /*!< indicates the state of the digital output */
    GPIO_TypeDef* doutput_port;   /*!< GPIO output port. It must be equal
                                   to GPIOx x= A, B, ...*/
    uint16_t doutput_pin;         /*!< GPIO output pin. It must be equal to
                                   GPIO_Pin_x x= 0, 1, ...*/
    uint8_t doutput_polarity;     /*!< GPIO output polarity. It must be equal
                                   to DOutputActiveHigh or DOutputActiveLow */
    void (*gpio_set_output_pin)(GPIO_TypeDef* TIMx, uint32_t pin_mask);
    void (*gpio_reset_output_pin)(GPIO_TypeDef* TIMx, uint32_t pin_mask);
} dout_t;
 ...
 * @retval none
 */
void dout_set_output_state(dout_t* phandle, doutput_state_t state);
