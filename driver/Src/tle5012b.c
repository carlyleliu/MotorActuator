...
static uint16_t SpiReadValue(uint16_t reg_value)
{
    ...
    HAL_SPI_Transmit(spi_handle, (uint8_t*)(&reg_value), sizeof(reg_value) / sizeof(uint16_t), 0xff);
    HAL_SPI_Receive(spi_handle, (uint8_t*)(&u16_data), sizeof(u16_data) / sizeof(uint16_t), 0xff);
